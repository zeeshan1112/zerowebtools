...
export default function SpinTheWheelWorkspace() {
  ...
  const updateItem = (idx: number, newValue: string) => {
    ...
    newItems[idx] = newValue || `Option ${idx + 1}`;
    setItems(newItems);
  };
                  ...
                  {items.map((item, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="flex items-center gap-2.5 group bg-surface/30 hover:bg-surface border border-transparent hover:border-border/30 rounded-xl px-2 py-1 transition-all"
                      >
                        <span
                        ...
                        <input
                          ...
                          disabled={spinning}
                          className="flex-1 text-sm font-medium text-ink bg-transparent border-none focus:outline-none focus:ring-0 py-1 min-w-0"
                        />
                        <button
                          onClick={() => removeItem(i)}
                          disabled={spinning || items.length <= 2}
                          className="p-2 -mr-1 rounded-lg text-ink-muted/40 hover:text-red-500 hover:bg-red-500/10 transition-all disabled:opacity-0 sm:opacity-0 sm:group-focus-within:opacity-100 sm:group-hover:opacity-100"
