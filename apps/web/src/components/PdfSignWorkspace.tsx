...
export default function PdfSignWorkspace() {
  ...
  const openPreview = async () => {
    ...
    setPreviewTitle("Preview Signed PDF");
    setShowPreview(true);
    setPreviewLoading(true);
    try {
      const pages = await renderAllPDFPages(signedBlob, 1.5);
  ...
  const handleDownload = () => {
    ...
    downloadBlob(signedBlob, outName);
  };
              ...
              <div className="space-y-2">
                {!signedBlob ? (
                  ...
                  <>
                    ...
                    <button
                      onClick={openPreview}
                      className="w-full py-2 border border-border bg-surface-elevated hover:bg-surface text-ink-secondary hover:text-ink rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" strokeWidth="2.5" stroke="currentColor">
                    ...
                    <button
                      onClick={() => setSignedBlob(null)}
                      className="w-full py-2 border border-dashed border-border text-ink-muted hover:text-ink rounded-xl text-xs font-bold transition-colors cursor-pointer"
                    >
                      Edit / Modify Signatures
                    </button>
                  </>
                )}
