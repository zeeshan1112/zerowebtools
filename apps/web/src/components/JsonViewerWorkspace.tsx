      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 bg-zinc-50/60 border-b border-border">
            <span className="text-xs font-medium text-ink-muted uppercase tracking-wider">Editor</span>
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 bg-zinc-50/60 border-b border-border">
            <span className="text-xs font-medium text-ink-muted uppercase tracking-wider">Tree View</span>
            {result.success && (
              <div className="flex items-center gap-3 text-xs text-ink-muted">
                <span>{result.stats.nodeCount} nodes</span>
                <span>depth {result.stats.depth}</span>
                <span>{result.stats.charCount} chars</span>
              </div>
            )}
          </div>
          ...
          <div className="h-[400px] lg:h-[520px] overflow-auto p-2 bg-surface-elevated font-mono text-sm">
            {!result.success ? (
              <div className="flex items-center justify-center h-full">
                <p className="text-sm text-red-500">{result.error || "Invalid JSON"}</p>
              </div>
            ) : result.tree.length === 0 ? (
              <div className="flex items-center justify-center h-full">
                <p className="text-sm text-zinc-400">Paste JSON to view the tree</p>
              </div>
            ) : (
